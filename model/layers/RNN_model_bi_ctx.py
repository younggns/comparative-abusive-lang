...
class SingleEncoderModelBiSingle:
    ...
    def _create_gru_model_bi(self):
        ...
        with tf.name_scope('text_RNN') as scope:
            ...
            with tf.variable_scope("text_GRU", reuse=False, initializer=tf.orthogonal_initializer()):
                ...
                cells_en_fw = tf.contrib.rnn.MultiRNNCell( [ self.gru_drop_out_cell() for _ in range(self.num_layers) ] )
                cells_en_bw = tf.contrib.rnn.MultiRNNCell( [ self.gru_drop_out_cell() for _ in range(self.num_layers) ] )
    ...
    def _add_context_gru(self):
        ...
        with tf.name_scope('text_RNN') as scope:
            ...
            with tf.variable_scope("text_GRU", reuse=True, initializer=tf.orthogonal_initializer()):
                ...
                cells_en_fw_c = tf.contrib.rnn.MultiRNNCell( [ self.gru_drop_out_cell() for _ in range(self.num_layers) ] )
                cells_en_bw_c = tf.contrib.rnn.MultiRNNCell( [ self.gru_drop_out_cell() for _ in range(self.num_layers) ] )
        ...
        # result merge
        self.final_encoder = tf.concat( [self.final_encoder, final_encoder_c], 1 )
    ...
    def _add_ff_layer(self):
        print ('[launch-text] add FF layer to reduce the dim: ', Params.DIM_FF_LAYER)
    ...
    def _create_output_layers(self):
        ...
        with tf.name_scope('loss') as scope:
            ...
            if ( self.consider_data_bias ) :
                ...
                self.class_weight = tf.constant( np.ones([3]) - norm, dtype=tf.float32 )
                self.weighted_logit = tf.multiply( self.batch_pred, self.class_weight )
                self.batch_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.weighted_logit, labels=self.y_labels )
