...
def ltc(batch_size, topic_size, memory_dim, input_hidden_dim, input_encoder, dr_memory_prob=1.0):
    ...
    with tf.compat.v1.name_scope('ltc') as scope:

        # memory space for latent topic
        memory = tf.compat.v1.get_variable("latent_topic_memory",
                                           shape=[topic_size, memory_dim],
                                           initializer=tf.compat.v1.orthogonal_initializer()
                                           )

        memory_W = tf.Variable(tf.random.uniform([input_hidden_dim, memory_dim],
                                                 minval=-0.25,
                                                 maxval=0.25,
                                                 dtype=tf.float32,
                                                 seed=None),
        ...
        memory_W = tf.nn.dropout(memory_W, 1 - (1 - (1 - (dr_memory_prob))))
        memory_bias = tf.Variable(
            tf.zeros([memory_dim], dtype=tf.float32), name="memory_projection_bias")
        ...
        # normalize
        topic_sim_sigmoid_softmax = tf.nn.softmax(logits=topic_sim, axis=-1)

        shaped_input = tf.reshape(topic_sim_sigmoid_softmax, [
                                  batch_size, topic_size])

        topic_sim_mul_memory = tf.scan(lambda a, x: tf.multiply(
            tf.transpose(a=memory), x), shaped_input, initializer=tf.transpose(a=memory))
        tmpT = tf.reduce_sum(
            input_tensor=topic_sim_mul_memory, axis=-1, keepdims=True)
