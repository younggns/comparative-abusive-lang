...
from layers.RNN_model_bi_ctx import *
...
def main(data_path, batch_size, encoder_size, num_layer, hidden_dim,
    ...
    model = SingleEncoderModelBiSingle(
...
if __name__ == '__main__':
    ...
    print('concat (context_text, ori_text) case')
    ...
    p.add_argument('--dbias', type=int, default=0)
    ...
    # graph_name = args.graph_prefix + '_D' + (args.data_path).split('/')[-2] + '_b' + str(args.batch_size) + '_es' + str(args.encoder_size) + '_L' + str(args.num_layer) + '_H' + str(args.hidden_dim) + '_G' + str(args.use_glove) + embed_train + '_dr' + str(args.dr) + '_o_type' + str(args.o_type) + '_c_text' + str(args.c_text) + '_c_type' + str(args.c_type) + '_attn' + str(args.attn) + '_ltc' + str(args.ltc) + '_dbias' + str(args.dbias)
